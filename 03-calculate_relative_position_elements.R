...
for (i in 1:dim(gnet_stimdata_matricesarray)[1]){
  ...
  if (!exists("gnet_stimdata_matricesarray_pairs")){
    gnet_stimdata_matricesarray_pairs <- pairs_data
  } else if (i == 2){
    gnet_stimdata_matricesarray_pairs <- abind(gnet_stimdata_matricesarray_pairs, pairs_data, along=0)
  } else {
    gnet_stimdata_matricesarray_pairs <- abind(gnet_stimdata_matricesarray_pairs, pairs_data, along=1)
  }
  rm(pairs_data)
}
rm(temp,i,j,k)